...
// we are been given two arrays , we have to find the XORed Value of all the elements in C , where C is made up of all the pairs of ( Ai | Bj ) [ i = 0 , 1.. n and j = 0 , 1.. n ]
void solve() {
	int n ; cin >> n ;
	vector<int> a ( n, 0) , b ( n , 0 ) , totol ( 32 , 0 ) , cnt ( 32 , 0 );
	...
	for ( int i = 0 ; i < n ; i ++) {
		for ( int j = 0 ; j < 32 ; j ++) {
			if ( (a.at(i) >> j ) & 1) totol.at(j) += n ;
			else totol.at(j) += cnt.at(j);
		}
	}
